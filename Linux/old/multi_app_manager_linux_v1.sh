...
# ============================================================================
# Multi-App Manager for Linux - V1
# ============================================================================
...
#
# PREREQUISITES:
# 1. Run this script as a user with sudo privileges (e.g., 'ubuntu').
# 2. Your application source code and a 'requirements.txt' should be ready.
#
# FEATURES:
# - Manages ROOT, SUBDOMAIN, SUBPATH, and STATIC sites.
# - Automated bootstrap for installing Nginx, Python, Certbot, UFW.
# - Centralized application configuration.
# - Generates systemd services for Python apps.
# - Generates and cleans Nginx server blocks automatically.
# - Full application lifecycle management (Deploy, Start, Stop, Logs, etc.).
# - SSL management via Certbot (including wildcard support).
# ============================================================================
...
function MainMenu() {
    while true; do
        clear
        echo "=================== Multi-App Manager V1 [Linux] ==================="
        echo
        echo " -- Initial Setup --"
...
function DeployedAppMenu() {
    while true; do
        ...
        echo "Current Status:"
        systemctl is-active --quiet "$CURRENT_SERVICE_NAME" && echo " ● Service is active (running)" || echo " ● Service is inactive (dead)"
        echo "=================================================================="
        echo
        ...
        case "$app_choice" in
            ...
            4)
                ...
                ;;
            5)
                # This is a streaming command, so no pause needed after it.
                echo "📜 Tailing logs. Press CTRL+C to stop."
                sudo journalctl -u "$CURRENT_SERVICE_NAME" -f
                ;;
            6) DeployApp; return ;;
            7)
...
function DeployApp() {
    ...
    if [[ "$CURRENT_TYPE" == "STATIC" ]]; then
        ...
        if [[ ! -f "${CURRENT_APP_FOLDER}/requirements.txt" ]]; then
            echo "[WARNING] 'requirements.txt' not found. Installing 'gunicorn'."
            "${CURRENT_VENV_PATH}/bin/pip" install --upgrade pip gunicorn
        else
            "${CURRENT_VENV_PATH}/bin/pip" install --upgrade pip
            "${CURRENT_VENV_PATH}/bin/pip" install -r "${CURRENT_APP_FOLDER}/requirements.txt"
        fi
...
EOF
    ...
    echo " - Reload Nginx to apply the new site config."
...
function GenerateNginxConfigFileForDomain() {
    ...
    # Create the config using a heredoc
    {
        ...
        for i in "${!APP_DOMAINS[@]}"; do
            if [[ "${APP_DOMAINS[$i]}" == "$target_domain" && "${APP_TYPES[$i]}" == "SUBPATH" ]]; then
                echo ""
...
function UninstallApp() {
    ...
    if [[ "$CURRENT_TYPE" != "STATIC" ]]; then
        ...
        sudo systemctl stop "$CURRENT_SERVICE_NAME" || true
        sudo systemctl disable "$CURRENT_SERVICE_NAME" || true
    ...
    if [[ "$CURRENT_TYPE" == "SUBPATH" ]]; then
        ...
        echo "            To remove it from Nginx, you MUST re-run 'Deploy'"
        echo "            for its parent domain: '$CURRENT_DOMAIN'"
    else
        echo "[INFO] This is a primary app. Removing its Nginx config..."
...
function ManageNginxMenu() {
    while true; do
        ...
        # Show current status at the top of the menu
        echo "Current Status:"
        systemctl is-active --quiet nginx && echo " ● Nginx is active (running)" || echo " ● Nginx is inactive (dead)"
        echo "-----------------------------------------------"
        echo " 1) Start Nginx"
        echo " 2) Stop Nginx"
        ...
        case "$nginx_choice" in
            ...
            3)
                ...
                # Use 'if' to show clear success/failure
...
function ManageSslMenu() {
    while true; do
        ...
        case "$ssl_choice" in
            1) echo "Stopping Nginx..."; sudo systemctl stop nginx; sudo certbot renew; sudo systemctl start nginx; echo "Done." ;;
            2) ObtainWildcardCert ;;
            0) return ;;
            *) echo "[ERROR] Invalid choice." ;;
        esac
        press_enter_to_continue
...
function SyncAndClean() {
    ...
    local registered_domains_conf=""
    for i in "${!APP_TYPES[@]}"; do
        type="${APP_TYPES[$i]}"
        if [[ "$type" == "ROOT" || "$type" == "SUBDOMAIN" || "$type" == "STATIC" ]]; then
            registered_domains_conf+="${APP_DOMAINS[$i]}.conf "
        fi
    done
    echo "[INFO] Active primary domains configs: $registered_domains_conf"

    for conf_path in ${NGINX_SITES_ENABLED}/*.conf; do
        if [[ -f "$conf_path" ]]; then
            conf_file=$(basename "$conf_path")
            # Check if conf_file is in the list of registered domains
            if ! echo " $registered_domains_conf " | grep -q " $conf_file "; then
                echo "[CLEANUP] Found orphaned config: $conf_file. Removing symlink..."
                sudo rm "$conf_path"
            fi
        fi
    done
...
function ObtainCertificate() {
    ...
    if ! sudo certbot certonly --standalone -d "$domain_to_cert" --non-interactive --agree-tos -m "$CERTBOT_EMAIL"; then
        echo "[ERROR] Certbot failed. Check logs."
        sudo systemctl start nginx
        return 1
    fi
    sudo systemctl start nginx
...
function ObtainWildcardCert() {
    ...
    echo "==========================================================================="
    sudo certbot certonly --manual --preferred-challenges=dns -d "*.${ROOT_DOMAIN}" -d "${ROOT_DOMAIN}" --agree-tos -m "${CERTBOT_EMAIL}"
...
function DisableDefaultNginxConfig() {
    local default_conf="${NGINX_SITES_ENABLED}/00-default"
    if [[ -L "$default_conf" ]]; then # Check if it's a symlink
...
function RestoreDefaultNginxConfigIfNeeded() {
    ...
    for i in "${!APP_TYPES[@]}"; do
        if [[ "${APP_TYPES[$i]}" == "ROOT" && "${APP_NAMES[$i]}" != "$uninstalled_app_name" ]]; then
    ...
    if (( another_root_exists )); then
        ...
        echo "[INFO] No other ROOT applications found. Re-enabling default Nginx page."
        sudo ln -sf "${NGINX_SITES_AVAILABLE}/00-default" "${NGINX_SITES_ENABLED}/00-default"
    fi
}
